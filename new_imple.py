...
def get_pretrained_model():
    ...
    keys = [k for k in hf_state if not k.endswith('attn.masked_bias')]
    for k in keys:
        if any(k.endswith(w) for w in transposed):
            assert hf_state[k].shape[::-1] == state[k].shape
            with torch.no_grad():
                state[k].copy_(hf_state[k].t())
        else:
            assert hf_state[k].shape == state[k].shape
            with torch.no_grad():
...
if __name__ == "__main__":
    ...
    y1 = model.generate(x1, max_new_tokens=1024, do_sample=False)[0]
